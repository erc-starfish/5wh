...
# read in data
adpo_props <- read.csv("adpo_props.csv")
adpo_props_facet <- read.csv("adpo_props_facet.csv")
...
adpo_props$func <- factor(adpo_props$func, levels = c("comitative", "associative", "instrumental", "ablative", "locative", "goal", "recipient", "possessive", "direct object", "subject")) 
...
df <- adpo_props_facet
d <- ggplot(df, aes(x=date, y=prop)) + geom_point(color="#3d62a6", size=1.0) + geom_smooth(fullrange=FALSE, se=TRUE, span=1.5, color="black", size=0.8) + facet_wrap(.~func, nrow=2) + theme_bw() + theme(axis.text=element_text(color="black")) + ylim(0,1) + 
 ...
 theme(axis.text.x=element_text(angle=60, hjust=1), strip.text=element_text(size=10.0), strip.background=element_blank())
...
ggsave("Figure5.pdf", p, height=3.5, width=5.0)
```
