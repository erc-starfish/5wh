...
ggsave("orality.pdf", height=4, width=6)
...
pronoun_plot <- function(mydata, country, panelID, type="regular") {
  ...
  if (country == "DR") {
    barchart <- barchart + ggtitle(paste0("(", panelID, ") Dominican Republic"))
  } else {
    barchart <- barchart + ggtitle(paste0("(", panelID, ") ", country))
  }
  ...
  if (type == "supp") {
    ...
    barchart <- barchart + scale_fill_manual(values = c("OVERT" = "#3d62a6", "NULL" = "#dfe2f7")) + scale_color_manual(values = c("DOC" = "black", "LIT" = "black", "SUPP" = "black")) + theme(axis.text.x = element_text(size = 9, color = "black"))
    #supp genre ^^^
  } else if (type == "regular") {
    barchart <- barchart + scale_fill_manual(values = c("NULL" = "#7294D4", "OVERT" = "#C6CDF7")) + scale_color_manual(values = c("DOC" = "black", "LIT" = "white")) + theme(axis.text.x = element_text(size = 9, color = "black"))
    #regular ^^^
  }
...
pp2 <- pronoun_plot(mydata, country="DR", type="supp", panelID="b")
pp3 <- pronoun_plot(mydata, country="Bolivia", type="supp", panelID="c")
#pp4 <- pronoun_plot(mydata, country="Panama", type="supp", panelID="d")
pp5 <- pronoun_plot(mydata, country="Spain", type="supp", panelID="e")

# combined plot
combined_plot <- grid.arrange(pp2, pp3, pp5, pp5, nrow=2, ncol=2)
combined_plot <- grid.arrange(pp1, combined_plot, nrow=2, ncol=1, heights=c(1.1,2))

# save to external file
ggsave("pronoun_combined.pdf", plot=combined_plot, height=10, width=9)
```
